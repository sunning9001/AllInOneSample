...
public class DynamicMBeanSupport implements DynamicMBean
{
  ...
  protected void addMBeanOperation( String name, String[] paramTypes, String[] paramNames, String[] paramDescs, String desc, String rtype, int type )
  {
  ...
  private String getType( String attName, boolean read, boolean write )
  {
    ...
    if( attributes.containsKey( attName ) )
    {
      ...
      if( read )
      {
        if( !temp.isReadable() ) allowed = false;
      }
      if( write )
      {
        if( !temp.isWritable() ) allowed = false;
      }
      if( !allowed ) return null;
      else return temp.getType();
    }

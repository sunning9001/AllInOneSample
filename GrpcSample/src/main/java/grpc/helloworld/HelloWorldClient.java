...
public class HelloWorldClient {
	...
	/** Say hello to server. */
  public 	ListenableFuture<HelloReply> futureGreet(String name) {
    logger.info("Will try to greet " + name + " ...");
    HelloRequest request = HelloRequest.newBuilder().setName(name).build();
   
    ListenableFuture<HelloReply> response =null;
	try {
   ...
   return response;
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			ListenableFuture<HelloReply> f = client.futureGreet(user);
			f.get();
		} finally {
			client.shutdown();
		}
